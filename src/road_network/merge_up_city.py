...
def near_boundary(p, bbox, margin):
    x, _, z = p
    return (
        abs(x - bbox["min_x"]) <= margin
        or abs(x - bbox["max_x"]) <= margin
        or abs(z - bbox["min_z"]) <= margin
        or abs(z - bbox["max_z"]) <= margin
    )
...
def nearest_boundary_normal(p, bbox):
    x, _, z = p
    distances = [
        (abs(x - bbox["min_x"]), (-1.0, 0.0, 0.0)),
        (abs(x - bbox["max_x"]), (1.0, 0.0, 0.0)),
        (abs(z - bbox["min_z"]), (0.0, 0.0, -1.0)),
        (abs(z - bbox["max_z"]), (0.0, 0.0, 1.0)),
    ]
...
def load_part(folder, boundary_margin):
    ...
    bbox = read_cache_header(cache_path)
...
def main():
    ...
    parser.add_argument(
        "--parts",
        nargs="+",
        default=["up_city_1", "up_city_2", "up_city_3", "up_city_4"],
    )
    ...
    parser.add_argument("--output", default=os.path.join("up_city", "road_graph.json"))
